...
mod test {
    ...
    async fn test_accelerated_bls12_381_g2_msm() {
        test_accelerated_precompile(|hint_writer, oracle_reader| {
            ...
            let accelerated_result = fpvm_bls12_g2_msm(&input, 22500, hint_writer, oracle_reader).unwrap();
            let native_result = execute_native_precompile(*bls12_381::g2_msm::PRECOMPILE.address(), input, 22500).unwrap();
