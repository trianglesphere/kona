...
RUN cd kona && \
  ...
  cargo build -Zbuild-std=core,alloc --workspace --bin kona --locked --profile release-client-lto --exclude kona-host --exclude kona-nexus --exclude kona-bin-utils --exclude kona-providers-alloy --exclude kona-providers-local --exclude kona-net && \
