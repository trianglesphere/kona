...
use tokio::sync::{broadcast::Sender as BroadcastSender, watch::Receiver};
...
use crate::{Behaviour, BlockHandler, BlockValidator, Broadcast, BuilderError, Driver, Handler};
...
impl From<crate::Config> for Builder {
    fn from(config: crate::Config) -> Self {
        Self::new()
            .with_peer_redial(config.redial)
            .with_rollup_config(config.rollup_config)
            .with_block_time(config.block_time)
            .with_peer_scoring(config.scoring)
            .with_peer_monitoring(config.monitor_peers)
            .with_unsafe_block_signer_receiver(config.unsafe_block_signer)
            .with_keypair(config.keypair)
            .with_timeout(config.timeout)
            .with_address(config.gossip_address)
            .with_config(config.gossip_config)
    }
}
...
impl Builder {
    ...
    /// Builds the [`Driver`].
    pub fn build(mut self) -> Result<Driver, BuilderError> {
        // Extract builder arguments
        let timeout = self.timeout.take().unwrap_or(Duration::from_secs(60));
        let keypair = self.keypair.take().ok_or(BuilderError::MissingKeyPair)?;
        let addr = self.gossip_addr.take().ok_or(BuilderError::GossipAddrNotSet)?;
        let signer_recv = self.signer.ok_or(BuilderError::MissingUnsafeBlockSigner)?;
        let rollup_config = self.rollup_config.take().ok_or(BuilderError::MissingRollupConfig)?;

        // Block Handler setup
        let validator = BlockValidator::new(rollup_config.clone(), signer_recv);
        let handler = BlockHandler::new(rollup_config, validator);

        // Construct the gossip behaviour
        let config = self.config.unwrap_or(crate::Config::default_inner_config());
        info!(
            target: "gossip",
        ...
        if let Some(scoring) = self.scoring {
            let block_time = self.block_time.ok_or(BuilderError::MissingL2BlockTime)?;
            let params = scoring
        ...
        let swarm = SwarmBuilder::with_existing_identity(keypair)
            ...
            .map_err(|_| BuilderError::TcpError)?
            .with_behaviour(|_| behaviour)
            .map_err(|_| BuilderError::WithBehaviourError)?
        ...
        let broadcast = Broadcast::new(payload_tx);

        Ok(Driver::new(swarm, addr, redialing, handler, broadcast))
    }
}
