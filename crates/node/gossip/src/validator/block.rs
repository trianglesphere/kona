...
use crate::{BlockValidationError, Validator};
...
impl BlockValidator {
    ...
    /// Creates a new [`BlockValidator`].
    pub const fn new(config: RollupConfig, receiver: Receiver<Address>) -> Self {
        Self { rollup_config: config, signer_recv: receiver, seen_hashes: BTreeMap::new() }
    }
    ...
    pub const fn validate_version_specific_payload(
        ...
        const fn validate_v3(
            ...
            if block.blob_gas_used != 0 {
                return Err(BlockValidationError::BlobGasUsed);
            }

            if block.excess_blob_gas != 0 {
                return Err(BlockValidationError::ExcessBlobGas);
            }

            if parent_beacon_block_root.is_none() {
                return Err(BlockValidationError::ParentBeaconRoot);
            }
...
impl Validator for BlockValidator {
    ...
    fn validate(&mut self, envelope: &OpNetworkPayloadEnvelope) -> Result<(), Self::Error> {
        ...
        if received != expected {
            return Err(BlockValidationError::BlockHash { expected, received });
        }
            ...
            self.seen_hashes.get_mut(&envelope.payload.block_number())
        {
            ...
            if seen_hashes_at_height.len() > Self::MAX_BLOCKS_TO_KEEP {
                return Err(BlockValidationError::TooManyBlocks {
            ...
            if seen_hashes_at_height.contains(&envelope.payload.block_hash()) {
                return Err(BlockValidationError::BlockSeen {
        ...
        if msg_signer != block_signer {
            return Err(BlockValidationError::Signer {
                expected: msg_signer,
                received: block_signer,
            });
        }
...
pub(crate) mod tests {
    ...
    use op_alloy_consensus::OpTxEnvelope;
    use op_alloy_rpc_types_engine::{
        OpExecutionPayload, OpExecutionPayloadV4, OpPayloadError, PayloadHash,
    };
    ...
    fn test_block_invalid_hash() {
        ...
        assert!(matches!(
            validator.validate(&envelope),
            Err(BlockValidationError::BlockHash { .. })
        ));
    }
    ...
    fn test_cannot_validate_same_block_twice() {
        ...
        assert!(matches!(
            validator.validate(&envelope),
            Err(BlockValidationError::BlockSeen { .. })
        ));
    }
    ...
    fn test_cannot_have_too_many_blocks_for_the_same_height() {
        ...
        let next_payloads = (0..=BlockValidator::MAX_BLOCKS_TO_KEEP)
            .map(|_| {
                let mut block = v1_valid_block();
        ...
        assert!(matches!(
            validator.validate(next_payloads.last().unwrap()),
            Err(BlockValidationError::TooManyBlocks { .. })
        ));
    }
    ...
    fn test_invalid_signature() {
        ...
        assert!(matches!(
            validator.validate(&envelope),
            Err(BlockValidationError::Signature { .. })
        ));
    }
    ...
    fn test_invalid_signer() {
        ...
        assert!(matches!(validator.validate(&envelope), Err(BlockValidationError::Signer { .. })));
    }
    ...
    fn test_block_invalid_base_fee() {
        ...
        assert!(matches!(
            validator.validate(&envelope),
            Err(BlockValidationError::InvalidBlock(_))
        ));
    }
    ...
    fn test_v2_non_empty_withdrawals() {
        ...
        assert!(matches!(
            validator.validate(&envelope),
            Err(BlockValidationError::InvalidBlock(OpPayloadError::NonEmptyL1Withdrawals))
        ));
    }
    ...
    fn test_v3_gas_params() {
        ...
        assert!(matches!(validator.validate(&envelope), Err(BlockValidationError::BlobGasUsed)));
        ...
        assert!(matches!(validator.validate(&envelope), Err(BlockValidationError::ExcessBlobGas)));
    }
