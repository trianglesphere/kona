...
func p2pSelfAndPeers(t devtest.T, out *MixedOpKonaPreset) {
		...
		go func(node *dsl.L2CLNode) {
			...
			for _, peer := range peers.Peers {
				...
				for _, node := range nodes {
					...
					if otherPeerInfo.PeerID == peer.PeerID && isKonaNode(t, otherCLRPC, otherCLName) {
...
func p2pPeersAndPeerStats(t devtest.T, out *MixedOpKonaPreset) {
	nodes := out.L2CLKonaNodes
	var wg sync.WaitGroup
	for _, node := range nodes {
		wg.Add(1)
...
func p2pBanPeer(t devtest.T, out *MixedOpKonaPreset) {
	nodes := out.L2CLKonaNodes
	for _, node := range nodes {
		clRPC, err := GetNodeRPCEndpoint(t.Ctx(), &node)
		require.NoError(t, err, "failed to get RPC endpoint for node %s", node.Escape().ID().Key())
