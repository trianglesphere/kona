...
The `kona-node` requires a few CLI flags, but they all have defaults for localhost development.

**Required flags (with defaults):**

- `--l1-eth-rpc <L1_ETH_RPC>` (default: `http://localhost:8545`)
  URL of the L1 execution client RPC API.
- `--l1-beacon <L1_BEACON>` (default: `http://localhost:5052`)
  URL of the L1 beacon API.
- `--l2-engine-rpc <L2_ENGINE_RPC>` (default: `http://localhost:8551`)
  URL of the engine API endpoint of an L2 execution client.
- `--l2-provider-rpc <L2_PROVIDER_RPC>` (default: `http://localhost:8546`)
  An L2 RPC URL.
...
Then, run the `kona-node` using Base's chain id - `8453`. Since all the RPC endpoints have defaults, you can run it without specifying them if you have local services running on the default ports:

```
kona-node node --chain 8453
```
...
kona-node node \
    ...
    --l2-provider-rpc http://127.0.0.1:8546
```
